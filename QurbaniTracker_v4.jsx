...
function HomeScreen({ state, onNav }) {
  ...
  const allS   = event.animals.flatMap(a => a.shareholders.map(s => ({...s, animalName: a.name})));
  ...
  const DIST_GROUPS = [
    { label:"Delivered", fn: s => s.deliveryStatus==="delivered", color:"var(--primary)" },
    { label:"Collected", fn: s => s.deliveryStatus==="collected", color:"var(--blue)"    },
    { label:"Pending",   fn: s => s.deliveryStatus==="pending",   color:"var(--red)"     },
  ];
  const PAY_GROUPS = [
  ...
  return (
    <div className="page" style={{paddingBottom:8}}>
      ...
      <div className="stat-grid">
        <div className="stat-card hero link" onClick={() => onNav("animals")}>
        ...
        <div className="stat-card gold link" onClick={() => onNav("shariks")}>
...
function AnimalsScreen({ state, onNav, onAddAnimal }) {
  const event = state.events.find(e => e.id === state.activeEventId);
  if (!event) return <div className="page"><div className="empty"><div className="empty-ico"><ICow size={52}/></div><div className="empty-txt">No event selected</div></div></div>;
  return (
    <div className="page">
      <div className="sec-header"><div className="sec-title">Animals</div><span className="sec-badge">{event.animals.length}</span></div>
      {event.animals.length===0 && <div className="empty"><div className="empty-ico"><ICow size={52}/></div><div className="empty-txt">No animals yet. Tap + to add one.</div></div>}
      {event.animals.map(a => {
        const kg=a.parts.reduce((s,p)=>s+Number(p.totalKg),0);
        const ps=perSlotKg(a);
        const del=a.shareholders.filter(s=>s.deliveryStatus!=="pending").length;
        return (
          <div className="animal-card" key={a.id} onClick={() => onNav("animalDetail",a.id)}>
            <div className="animal-card-body">
              <div className="animal-icon-wrap"><ICow size={26}/></div>
              <div style={{flex:1,minWidth:0}}>
                ...
                <div className="animal-share">≈ {ps.toFixed(2)} kg/unit · {del}/{a.shareholders.length} delivered</div>
              </div>
              <IChevR size={18} style={{color:"var(--text-3)",flexShrink:0}}/>
            </div>
            <div className="animal-prog"><div className="animal-prog-fill" style={{width:a.shareholders.length?`${del/a.shareholders.length*100}%`:"0%"}}/></div>
          </div>
        );
      })}
...
/* ══════════════════════════════════════════════════════════════════
   EVENTS SCREEN — with delete + confirm (Fix 6)
══════════════════════════════════════════════════════════════════ */
function EventsScreen({ state, onSetActive, onAddEvent, onDeleteEvent }) {
  const [showAdd, setShowAdd]   = useState(false);
  const [name, setName]         = useState(`Eid-ul-Adha ${new Date().getFullYear()}`);
  const [confirmDel, setConfirmDel] = useState(null); // event id
  return (
    <div className="page">
      ...
      {state.events.map(ev => {
        ...
        return (
          <div key={ev.id} className={`event-card ${isActive?"active-ev":""}`} onClick={()=>onSetActive(ev.id)}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
              <div style={{fontFamily:"Poppins,sans-serif",fontSize:16,fontWeight:700,color:"var(--text-1)",flex:1,minWidth:0}}>
              ...
              {/* FIX 6: delete button on each event */}
              <div className="del-btn" onClick={e=>{e.stopPropagation();setConfirmDel(ev.id);}}>
                <ITrash size={14}/>
              </div>
            </div>
            <div style={{fontSize:12,color:"var(--text-2)"}}>{ev.animals.length} animals · {total} shareholders</div>
          </div>
        );
      })}
      ...
      {/* FIX 6: Confirm before event delete */}
      {confirmDel && (
        <ConfirmModal
          title="Delete Event?"
...
export default function App() {
  ...
  const updateAnimal = useCallback(updated => {
    setState(prev => ({
      ...prev,
      events: prev.events.map(ev =>
        ev.id!==prev.activeEventId ? ev : { ...ev, animals: ev.animals.map(a => a.id===updated.id?updated:a) }
      )
    }));
  }, []);
  ...
  const deleteAnimal = useCallback(animalId => {
    setState(prev => ({
      ...prev,
      events: prev.events.map(ev =>
        ev.id!==prev.activeEventId ? ev : { ...ev, animals: ev.animals.filter(a=>a.id!==animalId) }
      )
    }));
    setScreen("animals");
  ...
  if (screen==="animalDetail" && activeEvent) {
    ...
    if (animal) return (
      <>
        <style>{CSS}</style>
        <Toast msg={toast}/>
        <AnimalDetail animal={animal} eventName={activeEvent.name} onBack={()=>setScreen("animals")} onUpdate={updateAnimal} onDeleteAnimal={deleteAnimal} toast={showToast}/>
      </>
    );
  }
      ...
      <div className="app">
        ...
        <div className="content-card">
          <div className="scroll-area">
            ...
            {screen==="events"  && <EventsScreen  state={state} onSetActive={id=>{setState(p=>({...p,activeEventId:id}));setScreen("home");showToast("Switched ✓");}} onAddEvent={addEvent} onDeleteEvent={deleteEvent}/>}
          </div>
